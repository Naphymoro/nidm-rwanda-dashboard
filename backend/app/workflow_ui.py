  ...
  <head>
    ...
    <style>
      ...
      * { min-width: 0; }
      ...
      .science-os .workspace-title-row .workspace-subtitle {
        max-width: 780px;
        color: var(--text);
        font-weight: 850;
      ...
      /* NDIM scientific workspace redesign: calmer shell, clearer navigation, lighter workbench. */
      :root {
        --bg: #eef2f4;
        --panel: #fffdf8;
        --panel-2: #f5f1ea;
        --card: #ffffff;
        --line: rgba(39, 54, 61, .14);
        --line-strong: #2d6263;
        --text: #182429;
        --muted: #546269;
        --soft: #7a858b;
      ...
      body {
        background:
          radial-gradient(circle at 14% 0%, rgba(45, 98, 99, .11), transparent 28%),
          linear-gradient(180deg, rgba(255,255,255,.54), transparent 260px),
          var(--bg);
      }
      :root[data-theme="dark"] body {
        background:
          radial-gradient(circle at 12% 0%, rgba(131, 208, 200, .12), transparent 30%),
          linear-gradient(180deg, rgba(255,255,255,.03), transparent 260px),
          var(--bg);
      }
      .app {
        --sidebar-w: 244px;
      ...
      .sidebar {
        background: linear-gradient(180deg, color-mix(in srgb, var(--panel) 96%, var(--bg)), var(--panel-2));
      }
      .brand {
        height: auto;
        min-height: 70px;
      ...
      .content {
        --content-x: clamp(14px, 2.3vw, 28px);
        padding: 18px var(--content-x) 28px;
      }
      .command-center {
        width: 100%;
        margin: 0 0 18px;
        ...
        border-radius: 10px;
        background: var(--panel);
        box-shadow: 0 18px 50px rgba(19, 35, 38, .08);
        overflow: hidden;
      }
      .command-center.science-os,
      :root[data-theme="dark"] .command-center.science-os {
      ...
      .science-os .workspace-launchpad {
        ...
        border-bottom: 1px solid var(--line);
        background:
          linear-gradient(135deg, color-mix(in srgb, var(--card) 86%, transparent), color-mix(in srgb, var(--panel-2) 72%, transparent));
      }
      .science-os .workspace-launchpad.compact-workspace {
      ...
      .skill-harness {
        border: 1px solid var(--line);
        border-radius: 8px;
        background: color-mix(in srgb, var(--card) 88%, var(--panel-2));
        padding: 12px;
      }
      .skill-harness.compact {
        margin-top: 14px;
      }
      .skill-harness-head {
        display: flex;
      ...
      .skill-harness-head h3 {
        margin: 3px 0 0;
        font-size: 16px;
      }
      .skill-harness-head p {
        margin: 0;
        color: var(--muted);
        font-size: 12.8px;
        line-height: 1.45;
      ...
      /* NDIM Research OS v2: a lighter agent workbench inspired by task, tool, memory, and trace surfaces. */
      :root {
        --bg: #f4f7f9;
        --panel: #ffffff;
        --panel-2: #f7f9fc;
      ...
      body {
        background:
          linear-gradient(180deg, color-mix(in srgb, var(--panel-2) 78%, transparent), transparent 340px),
          radial-gradient(circle at 78% -10%, rgba(37, 99, 235, .10), transparent 30%),
          var(--bg);
      ...
      :root[data-theme="dark"] .workspace-nav,
      :root[data-theme="dark"] .stepper,
      :root[data-theme="dark"] .sidebar-footer,
      :root[data-theme="dark"] .science-os .workspace-prime,
      :root[data-theme="dark"] .science-os .workspace-switcher,
      :root[data-theme="dark"] .science-os .agent-cockpit,
      :root[data-theme="dark"] .agent-bench,
      :root[data-theme="dark"] .os-stage-card,
      :root[data-theme="dark"] .os-repository-card,
      :root[data-theme="dark"] .os-assistant-card,
      :root[data-theme="dark"] .os-metric,
      ...
      :root[data-theme="dark"] .nav-item.primary,
      ...
      .content {
        --content-x: 12px;
        padding: 14px var(--content-x) 28px;
      }
      .command-center,
      .command-center.science-os,
      :root[data-theme="dark"] .command-center.science-os {
      ...
      .science-os .workspace-title-row {
        display: grid;
        grid-template-columns: minmax(0, 1fr) auto;
        gap: 12px;
      }
      ...
      .research-command {
        ...
        border-radius: 18px;
        background:
          linear-gradient(135deg, #101827 0%, #183252 58%, #204a60 100%);
        color: #edf5ff;
        box-shadow: 0 18px 46px rgba(16, 24, 39, .20);
      }
      :root[data-theme="dark"] .research-command {
        border-color: rgba(148, 163, 184, .20);
        background: linear-gradient(135deg, #070b13 0%, #10213a 68%, #123b45 100%);
      }
      .research-command .eyebrow {
        color: #9cc6ff;
      }
      .research-command h3 {
        margin: 5px 0 12px;
      ...
      .command-composer span {
        color: #7dd3fc;
        font: 900 16px var(--font-data);
      }
      .command-composer p {
        margin: 0;
        color: #e5edf7;
      ...
      .agent-bench.compact .agent-bench-grid {
        grid-template-columns: repeat(auto-fit, minmax(210px, 1fr));
      }
      .workspace-side-stack .agent-bench.compact .agent-bench-grid {
        grid-template-columns: 1fr;
      }
      .agent-bench.compact .agent-bench-head p {
        display: none;
      }
      .agent-bench.compact .agent-tile {
        min-height: 62px;
        grid-template-columns: 30px minmax(0, 1fr);
        padding: 9px;
      }
      .agent-bench.compact .agent-glyph {
        width: 30px;
        height: 30px;
        border-radius: 9px;
        font-size: 10px;
      }
      .agent-bench.compact .agent-tile p {
        display: none;
      }
      .agent-tile {
        display: grid;
      ...
      .app.workspace-start-mode {
        grid-template-columns: var(--sidebar-w) minmax(0, 1fr);
      }
      .app.workspace-start-mode .topbar {
        display: none;
      }
      .app.workspace-start-mode .sidebar {
        position: sticky;
      ...
      .app.workspace-start-mode .workspace-nav {
        display: grid;
      }
      .app.workspace-start-mode .sidebar-footer {
        display: grid;
      ...
      .app.workspace-start-mode .content {
        padding-top: 18px;
      }
      ...
      @media (max-width: 640px) {
        .content {
          --content-x: 10px;
          padding: 10px var(--content-x) 22px;
        }
        .topbar {
          margin: 8px 8px 0;
          align-items: stretch;
      ...
      /* DeerFlow reference pass: quiet rail, centered command thread, contextual panes. */
      .app.df-mode {
        display: block;
        width: 100%;
        min-height: 100vh;
      ...
      .app.df-mode .sidebar,
      .app.df-mode .topbar,
      .app.df-mode .stage-nav {
        display: none !important;
      }
      .app.df-mode .main,
      .app.df-mode .content,
      .app.df-mode #stagePanel {
        min-height: 100vh;
        width: 100%;
      }
      .app.df-mode .content {
      ...
      .df-chip .ui-icon,
      .df-send .ui-icon,
      .df-icon-button .ui-icon,
      .df-pane-action .ui-icon {
        width: 17px;
        height: 17px;
        flex: 0 0 auto;
      }
      ...
      .df-new-chat:hover,
      .df-new-chat:focus-visible,
      .df-nav-row:hover,
      .df-nav-row:focus-visible,
      .df-nav-row.active,
      .df-rail-row:hover,
      .df-rail-row:focus-visible,
      .df-rail-row.active {
      ...
      .df-deer-main {
        ...
        background:
          radial-gradient(circle at 50% 6%, color-mix(in srgb, var(--line-strong) 10%, transparent), transparent 28%),
          var(--bg);
      }
      .df-workbench-top {
        display: flex;
      ...
      .df-workbench-top strong {
        display: block;
        margin-top: 2px;
        color: var(--text);
        font-size: 14px;
      }
      .df-workbench-actions {
        display: inline-flex;
        align-items: center;
        gap: 8px;
        flex-wrap: wrap;
      ...
      .df-main-center {
        ...
        margin: 0 auto;
        padding: clamp(38px, 8vh, 92px) 24px 46px;
      }
      .df-active-workbench .df-main-center {
      ...
      .df-active-workbench .df-center-copy h1 {
      ...
      .df-skill.active,
      .df-phase.active {
        border-color: color-mix(in srgb, var(--line-strong) 50%, var(--line));
        background: color-mix(in srgb, var(--line-strong) 9%, var(--card));
      }
      .df-artifact-grid,
      .df-pipeline-grid {
        display: grid;
      ...
      .df-mode-body .floating-assistant {
        width: 46px;
        height: 46px;
        right: 18px;
        bottom: 18px;
      ...
      .df-mode-body .floating-assistant span {
        display: none;
      }
      :root[data-theme="dark"] .app.df-mode {
        background: #1f201d;
      }
      :root[data-theme="dark"] .df-workbench.df-flow-app,
      :root[data-theme="dark"] .df-deer-main {
        background: #1f201d;
      }
      :root[data-theme="dark"] .df-deer-rail {
        background: #1b1c1a;
      ...
      :root[data-theme="dark"] .df-prompt-box,
      :root[data-theme="dark"] .df-current-lane,
      :root[data-theme="dark"] .df-pane,
      :root[data-theme="dark"] .df-skill,
      :root[data-theme="dark"] .df-stage-row,
      :root[data-theme="dark"] .df-artifact,
      :root[data-theme="dark"] .df-phase,
      ...
      :root[data-theme="dark"] .df-new-chat,
      :root[data-theme="dark"] .df-nav-row:hover,
      :root[data-theme="dark"] .df-nav-row:focus-visible,
      :root[data-theme="dark"] .df-nav-row.active,
      :root[data-theme="dark"] .df-rail-row:hover,
      :root[data-theme="dark"] .df-rail-row:focus-visible,
      :root[data-theme="dark"] .df-rail-row.active {
        background: #282925;
        border-color: rgba(255,255,255,.1);
      }
      :root[data-theme="dark"] .df-center-copy h1,
      :root[data-theme="dark"] .df-rail-brand strong,
      :root[data-theme="dark"] .df-workbench-top strong,
      :root[data-theme="dark"] .df-rail-row strong,
      :root[data-theme="dark"] .df-skill strong,
      :root[data-theme="dark"] .df-stage-row strong,
      :root[data-theme="dark"] .df-artifact strong,
      :root[data-theme="dark"] .df-phase strong,
      :root[data-theme="dark"] .df-insight-grid strong,
      :root[data-theme="dark"] .df-metric-strip strong {
        color: #f4f3ee;
      }
      :root[data-theme="dark"] .df-center-copy p,
      :root[data-theme="dark"] .df-prompt-box > p,
      :root[data-theme="dark"] .df-pane-note,
      :root[data-theme="dark"] .df-current-lane p,
      :root[data-theme="dark"] .df-skill small,
      :root[data-theme="dark"] .df-stage-row small,
      :root[data-theme="dark"] .df-artifact small,
      :root[data-theme="dark"] .df-phase small,
      :root[data-theme="dark"] .df-trace-line p {
        color: #b9bbb4;
      }
  ...
  <body>
    <div class="app" id="appShell">
      <aside class="sidebar">
        ...
        <nav class="workspace-nav" aria-label="NDIM workspace sections">
          <p class="workspace-nav-title">Workspace</p>
          <button class="nav-item primary" data-start-toggle type="button">Start workspace</button>
          <button class="nav-item" data-flow-step="0" type="button">Evidence</button>
          <button class="nav-item" data-flow-step="3" type="button">Encode</button>
    ...
    <script>
      ...
      const workflowPhases = [
        ...
        { icon: "TW", label: "Twin", detail: "Feedback, posterior, RL loop", step: 6, stages: ["digital", "bayes", "rl"] },
        { icon: "ST", label: "Strategy", detail: "Regions, graph, inoculation lab", step: 9, stages: ["regional", "graph", "inoculation"] },
      ...
      const ndimSkills = [
        {
          ...
          agent: "Field evidence steward",
          step: 0,
          stages: ["intake", "gate", "repository"],
          summary: "Checks source, place, consent, visibility, approvals, hashes, and readiness before a story enters the model."
        },
        {
          id: "narrative-encoding",
          label: "Narrative Encoding",
          agent: "Human-AI encoder",
          step: 3,
          stages: ["encoding"],
          summary: "Turns stories into transparent trust, barrier, exposure, and inoculation signals with manual, AI, or hybrid scoring."
        },
        {
          id: "math-epidemiology",
          label: "Mathematics & Epidemiology",
          agent: "Epidemiology modeller",
          step: 4,
          stages: ["compartmental", "agents", "digital", "bayes", "rl"],
          summary: "Helps build SI/SIS/SIR/SEIR-style, rumour, network, agent, Bayesian, and behavioural diffusion models, then explains coefficients and assumptions."
        },
        {
          ...
          step: 3,
          stages: ["encoding", "agents", "inoculation"],
          summary: "Reads trust, memory, fear, habits, social proof, messenger credibility, and barriers as model inputs rather than loose commentary."
        },
        {
          ...
          step: 11,
          stages: ["regional", "graph", "inoculation", "policy"],
          summary: "Designs online and local campaign strategies: message framing, audience segmentation, trusted messengers, channels, and validation."
        },
        {
          id: "digital-twin",
          label: "Digital Twin Simulation",
          agent: "Feedback simulator",
          step: 6,
          stages: ["digital", "bayes", "rl"],
          summary: "Compares predictions with field feedback, updates uncertainty, and shows what changes when trust or barriers move."
        },
        {
          id: "publication",
          label: "Publication",
          ...
          step: 12,
          stages: ["policy"],
          summary: "Prepares methods, limitations, evidence trail, references, LaTeX, DOCX, PDF, and Frontiers-style manuscript exports."
        },
        {
          id: "literature-search",
          label: "Literature Search",
          agent: "Open-source scholar",
          step: 12,
          stages: ["policy"],
          summary: "Supports careful, legal discovery of open literature for epidemiology, misinformation, diffusion, inoculation, and policy modelling."
        }
      ];
      ...
      const LOCAL_WORKSPACES_KEY = "ndim_local_workspaces_v1";
      const LOCAL_ACTIVE_WORKSPACE_KEY = "ndim_local_active_workspace_v1";
      ...
      function localWorkspaceSettings(country = "Rwanda", domain = "general narrative evidence") {
        const language = country === "Rwanda" ? "rw" : "en";
        return {
          countries: [country || "Rwanda"],
          admin_unit_mode: "country_schema",
          domain: domain || "general narrative evidence",
          enabled_evidence_routes: evidenceModes.map((mode) => mode.id),
      ...
      function localWorkspaceRecord(payload) {
        const workspaceId = payload.workspace_id || slugifyWorkspace(payload.name);
        const country = payload.country || "Rwanda";
        const domain = payload.domain || "general narrative evidence";
        return {
          schema: "ndim-workspace-v1",
          id: workspaceId,
          workspace_id: workspaceId,
          name: payload.name || "Untitled NDIM Workspace",
          description: payload.description || `${payload.name || "Untitled NDIM Workspace"} NDIM workspace.`,
          domain,
          countries: [country],
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
          version: 1,
          settings: localWorkspaceSettings(country, domain)
        };
      }

      function builtInLocalWorkspaces() {
        return [
      ...
      function readLocalWorkspaces() {
        let stored = [];
        try {
          stored = JSON.parse(window.localStorage?.getItem(LOCAL_WORKSPACES_KEY) || "[]");
        } catch (_) {
          stored = [];
        }
        const byId = new Map();
        [...builtInLocalWorkspaces(), ...stored].forEach((workspace) => {
          if (workspace?.workspace_id) byId.set(workspace.workspace_id, workspace);
        });
        return [...byId.values()];
      }

      function writeLocalWorkspaces(workspaces) {
        const byId = new Map();
        workspaces.forEach((workspace) => {
          if (workspace?.workspace_id) byId.set(workspace.workspace_id, {
            ...workspace,
            id: workspace.workspace_id,
            countries: workspace.countries || workspace.settings?.countries || [],
            domain: workspace.domain || workspace.settings?.domain || "",
            updated_at: new Date().toISOString()
          });
        });
        window.localStorage?.setItem(LOCAL_WORKSPACES_KEY, JSON.stringify([...byId.values()]));
      }

      function upsertLocalWorkspace(workspace) {
        const workspaces = readLocalWorkspaces().filter((item) => item.workspace_id !== workspace.workspace_id);
        workspaces.push(workspace);
        writeLocalWorkspaces(workspaces);
        return workspace;
      }

      function createLocalWorkspace(payload) {
        const existingIds = new Set(readLocalWorkspaces().map((workspace) => workspace.workspace_id));
        const base = slugifyWorkspace(payload.name);
        let workspaceId = base;
        let suffix = 2;
        while (existingIds.has(workspaceId)) {
          workspaceId = `${base}-${suffix}`;
          suffix += 1;
        }
        const workspace = localWorkspaceRecord({ ...payload, workspace_id: workspaceId });
        upsertLocalWorkspace(workspace);
        window.localStorage?.setItem(LOCAL_ACTIVE_WORKSPACE_KEY, workspace.workspace_id);
        return workspace;
      }

      function setLocalActiveWorkspace(workspaceId) {
        const workspaces = readLocalWorkspaces();
        const workspace = workspaces.find((item) => item.workspace_id === workspaceId);
        if (!workspace) throw new Error("Workspace not found");
        window.localStorage?.setItem(LOCAL_ACTIVE_WORKSPACE_KEY, workspace.workspace_id);
        state.workspace.active = workspace;
        state.workspace.list = workspaces;
      ...
      async function loadWorkspaceState() {
        try {
          ...
          const workspaces = readLocalWorkspaces();
          const savedActive = window.localStorage?.getItem(LOCAL_ACTIVE_WORKSPACE_KEY) || "ndim-core";
          const active = workspaces.find((workspace) => workspace.workspace_id === savedActive) || workspaces[0];
          state.workspace.active = active;
          state.workspace.list = workspaces;
          applyWorkspaceDefaults(active);
          trace("workspace", "Browser-local workspace mode", "The backend workspace service is unavailable, so NDIM is using the browser-local workspace list for this session.");
        }
      }

      async function setActiveWorkspace(workspaceId, options = {}) {
        ...
        try {
          ...
          trace("workspace", "Browser-local workspace selected", `${workspaceName()} is active in local browser storage.`);
        }
      ...
      async function createWorkspaceFromManager() {
        ...
        let workspace;
        try {
          workspace = await apiJson("/workspaces", { method: "POST", body: JSON.stringify(payload) });
        } catch (error) {
          workspace = createLocalWorkspace(payload);
          trace("workspace", "Workspace created locally", "The backend workspace endpoint was not available, so the workspace was saved in browser-local storage.");
        }
        await setActiveWorkspace(workspace.workspace_id, { renderAfter: false });
      ...
      async function duplicateWorkspaceById(sourceId) {
        ...
        let workspace;
        try {
          workspace = await apiJson(`/workspaces/${encodeURIComponent(sourceId)}/duplicate`, {
            method: "POST",
            body: JSON.stringify({ name: defaultName })
          });
        } catch (error) {
          const base = (readLocalWorkspaces().find((item) => item.workspace_id === sourceId) || source || state.workspace.active || {});
          workspace = createLocalWorkspace({
            name: defaultName,
            description: base.description || `Copy of ${source?.name || sourceId}.`,
            country: (base.settings?.countries || base.countries || ["Rwanda"])[0],
            domain: base.settings?.domain || base.domain || "general narrative evidence"
          });
        }
        await setActiveWorkspace(workspace.workspace_id, { renderAfter: false });
      ...
      function renderSkillHarness(compact = false) {
        const activeStage = currentStep().id;
        return `
          <section class="skill-harness ${compact ? "compact" : ""}" aria-label="NDIM skill harness">
            <div class="skill-harness-head">
              <div>
                <p class="eyebrow">Skill harness</p>
            ...
            <div class="skill-grid">
              ${ndimSkills.map((skill) => `
                <button class="skill-card ${skill.stages.includes(activeStage) ? "active" : ""}" data-flow-step="${skill.step}" type="button" title="${escapeHtml(skill.summary)}">
                  <span>${escapeHtml(skill.agent)}</span>
      ...
      function renderAgentBench(compact = false) {
        const activeStage = state.workspace.started ? currentStep().id : "";
        const visibleSkills = compact ? ndimSkills.slice(0, 5) : ndimSkills;
        const status = state.workspace.started ? "Workspace memory active" : "Waiting for workspace";
        return `
          <section class="agent-bench ${compact ? "compact" : ""}" aria-label="NDIM agent and skill bench">
            <div class="agent-bench-head">
              <div>
                <p class="eyebrow">Agent harness</p>
                <h3>Specialist skills inside NDIM</h3>
                <p>Evidence, mathematics, epidemiology, behavioural science, strategy, twin simulation, and publication support work from the same traceable workspace.</p>
              </div>
              <span class="bench-status">${escapeHtml(status)}</span>
            </div>
            <div class="agent-bench-grid">
              ${visibleSkills.map((skill) => `
                <button class="agent-tile ${skill.stages.includes(activeStage) ? "active" : ""}" data-flow-step="${skill.step}" type="button" title="${escapeHtml(skill.summary)}">
                  <span class="agent-glyph">${escapeHtml(skill.label.split(" ").map((part) => part[0]).join("").slice(0, 2))}</span>
                  <span>
                    <em>${escapeHtml(skill.agent)}</em>
                    <strong>${escapeHtml(skill.label)}</strong>
                    <p>${escapeHtml(skill.summary)}</p>
                  </span>
                </button>
      ...
      function renderResearchCommand(started = false) {
        const step = state.workspace.started ? currentStep() : null;
        const commandText = started && step
          ? `Continue ${step.num} ${workspaceStageLabel(step)} for ${workspaceName()}: ${assistantNextAction()}`
          : "Describe the behaviour, rumour, adoption barrier, campaign pressure, or trust question you want NDIM to investigate.";
        const routeText = activeEvidenceModes().map((mode) => mode.title).slice(0, 4).join(" / ") || "Core narrative routes";
        return `
          <section class="research-command" aria-label="NDIM research command workspace">
            <div class="research-command-main">
              <p class="eyebrow">${started ? "Current command" : "Research command"}</p>
              <h3>${started ? "Move the next task through the engine" : "Start with a question, then load evidence"}</h3>
              <div class="command-composer" role="note">
                <span aria-hidden="true">&gt;</span>
                <p>${escapeHtml(commandText)}</p>
              </div>
              <div class="command-tags" aria-label="NDIM pipeline">
                <span>Evidence</span>
                <span>Models</span>
                <span>Intervention tests</span>
                <span>Decision support</span>
              </div>
            </div>
            <div class="research-runtime" aria-label="Workspace runtime">
              <div class="runtime-item"><b></b><span><strong>Memory</strong>${escapeHtml(started ? workspaceName() : "Workspace not loaded")}</span></div>
              <div class="runtime-item"><b></b><span><strong>Routes</strong>${escapeHtml(routeText)}</span></div>
              <div class="runtime-item"><b></b><span><strong>Skill focus</strong>${escapeHtml(started && step ? stageAdvice(step.id) : "Load a project container before running the scientific workflow.")}</span></div>
            </div>
          </section>
        `;
      }
      ...
      function renderDfWorkspaceRows(limit = 8) {
        const workspaces = state.workspace.list || [];
        return `
          <button class="df-rail-row new" data-start-action="create" type="button">
            <span class="df-rail-row-icon">${icon("plus")}</span>
            <span><strong>From scratch</strong><small>Start a clean study workspace.</small></span>
          </button>
          ${workspaces.slice(0, limit).map((item) => `
            <button class="df-rail-row ${item.workspace_id === workspaceId() ? "active" : ""}" data-workspace-open-quick="${escapeHtml(item.workspace_id)}" type="button">
              <span class="df-rail-row-icon">${item.workspace_id === workspaceId() ? "ON" : "GO"}</span>
      ...
      function renderDfRail(started = false) {
        const active = state.workspace.started ? currentStep().id : "";
        const navItems = [
          ["Evidence", 0, "evidence"],
          ["Encode", 3, "manual"],
          ["Model", 4, "model"],
          ["Digital twin", 6, "graph"],
          ["Strategy lab", 11, "policy"]
        ];
        return `
          <aside class="df-deer-rail" aria-label="NDIM workspace rail">
            <div class="df-rail-brand">
              <span class="df-brand-mark">ND</span>
              <span><strong>NDIM Engine</strong><small>Narrative Diffusion and Inoculation Model</small></span>
            </div>
            <button class="df-new-chat" data-start-action="create" type="button">${icon("plus")} New investigation</button>
            <nav class="df-rail-nav" aria-label="NDIM primary sections">
              <button class="df-nav-row ${!started ? "active" : ""}" data-start-toggle type="button">${icon("folder")} Workspaces</button>
              ${navItems.map(([label, index, iconName]) => `
                <button class="df-nav-row ${steps[index]?.id === active ? "active" : ""}" data-flow-step="${index}" type="button">${icon(iconName)} ${escapeHtml(label)}</button>
              `).join("")}
              <a class="df-nav-row" href="publication/" target="_blank" rel="noreferrer">${icon("export")} Publication</a>
              <a class="df-nav-row" href="academy/" target="_blank" rel="noreferrer">${icon("learn")} Academy</a>
              <a class="df-nav-row" href="manual/" target="_blank" rel="noreferrer">${icon("manual")} Manual</a>
            </nav>
            <div class="df-rail-section">
              <p>Workspace memory</p>
              <div class="df-rail-list">${renderDfWorkspaceRows(started ? 5 : 8)}</div>
            </div>
            <div class="df-rail-bottom">
              <button class="df-nav-row" data-open-workspace-manager type="button">${icon("settings")} Workspace manager</button>
              <button class="df-nav-row" data-workbench-theme-toggle type="button">${icon("moon")} Theme</button>
            </div>
          </aside>
        `;
      }
      ...
      function renderDfStageQueue() {
        return `
          <div class="df-stage-list" aria-label="NDIM workflow queue">
      ...
      function renderDfPipeline() {
        return `
          <div class="df-pipeline-grid" aria-label="NDIM pipeline">
      ...
      function renderDfSkillDock(limit = 8) {
        const activeStage = state.workspace.started ? currentStep().id : "";
        return `
          <div class="df-skill-list" aria-label="Specialist skill dock">
      ...
      function renderDfArtifactDock() {
        return `
          <div class="df-artifact-grid" aria-label="Learning and output artifacts">
            ...
            <a class="df-artifact" href="publication/" target="_blank" rel="noreferrer">${icon("export")}<span><strong>Publication</strong><small>Manuscript workspace</small></span></a>
      ...
      function renderDfTraceDock() {
        const logs = (state.trace || []).slice(-5).reverse();
        return `
          <div class="df-trace-list" aria-label="Recent run trace">
            <button class="df-pane-action" id="workbenchTraceButton" type="button">${icon("log")} Open full activity log</button>
      ...
      function renderDfPane() {
        const pane = state.workspace.activePane || "agents";
        if (!state.workspace.activePane) return "";
        const tabs = [
          ["agents", "Agents"],
          ["runs", "Runs"],
          ["artifacts", "Artifacts"],
        ...
        const bodies = {
          agents: `<p class="df-pane-note">Specialist agents translate the same evidence into governed narratives, behavioural signals, epidemiological models, strategy options, and publication-ready outputs.</p>${renderDfSkillDock(10)}`,
          runs: `<p class="df-pane-note">Open a stage as a focused task. The queue stays compact so the main screen can remain a thinking space.</p>${renderDfStageQueue()}`,
          artifacts: `<p class="df-pane-note">Open the learning materials, manuscript workspace, or evidence repository without leaving the NDIM command surface.</p>${renderDfArtifactDock()}`,
          activity: renderDfTraceDock()
        };
        return `
          <section class="df-pane" aria-label="Workbench context">
            <div class="df-pane-tabs">
              ${tabs.map(([id, label]) => `<button class="${pane === id ? "active" : ""}" data-workbench-pane="${id}" type="button">${escapeHtml(label)}</button>`).join("")}
            </div>
            <div class="df-pane-body">${bodies[pane] || bodies.agents}</div>
          </section>
        `;
      }

      function renderDfActionChips(started = false) {
        const runAttrs = started ? 'data-open-stage-workspace' : 'data-start-toggle';
        return `
          <div class="df-action-chips" aria-label="NDIM quick actions">
            <button class="df-chip" data-start-action="create" type="button">${icon("plus")} Create</button>
            <button class="df-chip" ${runAttrs} type="button">${icon("evidence")} Evidence</button>
            <button class="df-chip" data-flow-step="3" type="button">${icon("manual")} Encode</button>
            <button class="df-chip" data-flow-step="4" type="button">${icon("model")} Model</button>
            <button class="df-chip" data-flow-step="11" type="button">${icon("policy")} Strategy</button>
            <button class="df-chip" data-workbench-pane="runs" type="button">${icon("log")} Runs</button>
            <button class="df-chip" data-workbench-pane="agents" type="button">${icon("assistant")} Agents</button>
            <a class="df-chip" href="academy/" target="_blank" rel="noreferrer">${icon("learn")} Learn</a>
          </div>
        `;
      }

      function renderDfTopbar(started = false) {
        return `
          <div class="df-workbench-top">
            <div>
              <span>${escapeHtml(started ? "Active workspace" : "Workspace setup")}</span>
              <strong>${escapeHtml(started ? workspaceName() : "Choose or create a workspace")}</strong>
            </div>
            <div class="df-workbench-actions">
              ${dfStatusPill(started ? `${workspaceReadinessPercent()}% ready` : "not loaded", started)}
              <button class="df-icon-button" data-open-workspace-manager type="button" title="Workspace manager" aria-label="Workspace manager">${icon("settings")}</button>
              <button class="df-icon-button" data-open-assistant-panel type="button" title="Open research assistant" aria-label="Open research assistant">${icon("assistant")}</button>
              <button class="df-icon-button" data-workbench-theme-toggle type="button" title="Toggle theme" aria-label="Toggle theme">${icon("moon")}</button>
            </div>
          </div>
        `;
      }
      ...
      function renderDfCommandThread(started = false) {
        ...
        const lead = started ? `${step.num} ${workspaceStageLabel(step)}` : "What should NDIM investigate?";
        const question = started
          ? assistantNextAction()
          : "Describe the behaviour, rumour, trust question, adoption barrier, campaign pressure, or policy decision you want to examine.";
        const context = started
          ? workspaceSummaryText()
          : "NDIM Engine turns community narratives into scientific evidence for designing better interventions. It helps researchers and decision-makers understand how trust, lived experience, social influence, and public messages shape adoption, resistance, and behaviour change across communities.";
        return `
          <section class="df-thread-card" aria-label="NDIM command thread">
            <div class="df-center-copy">
              <p class="eyebrow">Narrative Diffusion and Inoculation Model</p>
              <h1>${escapeHtml(started ? lead : "NDIM Engine")}</h1>
              <p>${escapeHtml(started ? context : "A Scientific Platform for Understanding Narratives and Social Influence")}</p>
              ${started ? "" : `<p>It brings together field evidence, social data, modelling, uncertainty analysis, and digital twin simulations to show how narratives move through communities and how they may influence behaviour.</p>`}
            </div>
            <div class="df-prompt-box">
              <p>${escapeHtml(question)}</p>
              <div class="df-prompt-footer">
                <span>${icon("assistant")} NDIM command thread</span>
                <div>
                  ${started ? `<button class="df-send" id="workbenchRunButton" type="button">${icon("start")} Run stage</button><button class="df-send" data-open-stage-workspace type="button">Open focused task</button>` : `<button class="df-send" data-start-action="create" type="button">${icon("plus")} Start from scratch</button><button class="df-send" data-start-toggle type="button">Open existing</button>`}
                </div>
              </div>
            </div>
            ${renderDfActionChips(started)}
            <p class="df-disclaimer">Evidence -> Models -> Intervention Testing -> Decision Support</p>
          </section>
        `;
      }
      ...
      function renderDfActiveStage() {
        const step = currentStep();
        return `
          <section class="df-current-lane" aria-label="Active stage control">
            <div>
            ...
            <div class="df-current-actions">
              ${dfStatusPill(stageCompletionLabel(step.id), state.completed.has(step.id))}
              <button class="df-send" data-open-stage-workspace type="button">${icon("folder")} Open task</button>
              <button class="df-send" data-run-stage-direct type="button">${escapeHtml(stagePrimaryActionLabel(step.id))}</button>
            </div>
            <div class="df-insight-grid">
              <div><span>Input</span><strong>${escapeHtml(stageInputSummary(step.id))}</strong></div>
              <div><span>Scientific check</span><strong>${escapeHtml(scientificCautionText())}</strong></div>
              <div><span>Next</span><strong>${escapeHtml(assistantNextAction())}</strong></div>
            </div>
          </section>
        `;
      }
      ...
      function renderDfStartWorkbench() {
        return `
          <section class="df-workbench df-flow-app df-start-workbench" aria-label="NDIM agent workbench">
            ${renderDfRail(false)}
            <main class="df-deer-main">
              ${renderDfTopbar(false)}
              <div class="df-main-center">
      ...
      function renderDfActiveWorkbench() {
        return `
          <section class="df-workbench df-flow-app df-active-workbench" aria-label="NDIM active agent workbench">
            ${renderDfRail(true)}
            <main class="df-deer-main">
              ${renderDfTopbar(true)}
              <div class="df-main-center">
      ...
      function renderStrategyStrip() {
        return `
          <div class="strategy-strip" aria-label="Strategic communication lab">
            <div class="strategy-chip"><strong>Audience</strong>Segment by trust, exposure, barrier, place, and messenger relationship.</div>
            <div class="strategy-chip"><strong>Message</strong>Compare framing, prebunking, reassurance, proof, and call-to-action options.</div>
            <div class="strategy-chip"><strong>Channel</strong>Plan local meetings, radio, WhatsApp, social media, schools, clinics, markets, or extension visits.</div>
            <div class="strategy-chip"><strong>Validation</strong>Test meaning, safety, and unintended effects before policy or campaign use.</div>
          </div>
        `;
      }
      ...
      function renderWorkspaceLaunchpad() {
        ...
        const subtitle = "A Scientific Platform for Understanding Narratives and Social Influence";
        const summary = "NDIM Engine turns community narratives into scientific evidence for designing better interventions. It helps researchers and decision-makers understand how trust, lived experience, social influence, and public messages shape adoption, resistance, and behaviour change across communities.";
        const modelText = "It brings together field evidence, social data, modelling, uncertainty analysis, and digital twin simulations to show how narratives move through communities and how they may influence behaviour.";
        const scopeText = "NDIM Engine is designed for work in health, climate, agriculture, energy, and public policy. It helps users turn community narratives into structured evidence, test different intervention messages before using them in the real world, and produce transparent outputs that support policy and programme decisions.";
        const pathway = "Evidence \u2192 Models \u2192 Intervention Testing \u2192 Decision Support";
        if (state.workspace.started) {
          return `
            <div class="workspace-launchpad workspace-os compact-workspace" aria-label="Active NDIM workspace">
              <section class="workspace-prime">
                <div class="workspace-title-row">
                  <div>
                    <p class="eyebrow">Active workspace</p>
                    <h2>${escapeHtml(workspaceName())}</h2>
                    <p class="workspace-subtitle">${escapeHtml(workspaceSummaryText())}</p>
                    <p class="workspace-support-line">${escapeHtml(pathway)}</p>
                  </div>
                  <span class="workspace-badge">${escapeHtml(evidenceGrade())}</span>
                </div>
                <div class="workspace-command-row">
                  <div class="start-control">
                    <button class="button primary start-button" data-start-toggle type="button">${icon("folder")} Switch workspace</button>
                    ${renderStartMenu()}
                  </div>
                ...
                <div class="workspace-signal-row" aria-label="Workspace evidence routes">
                  ...
                  <span class="workspace-signal"><strong>Storage</strong> local-first</span>
                  <span class="workspace-signal"><strong>Assistant</strong> contextual</span>
                </div>
              </section>
              <div class="workspace-side-stack">
                ${renderAgentBench(true)}
              </div>
            </div>
          `;
        }
        return `
          <div class="workspace-launchpad workspace-os landing-workspace" aria-label="Workspace launchpad">
            <section class="workspace-prime">
              <div class="workspace-title-row">
                <div>
                  <p class="eyebrow">Narrative Diffusion and Inoculation Model</p>
                  <h2>${escapeHtml(title)}</h2>
                  <p class="workspace-subtitle">${escapeHtml(subtitle)}</p>
              ...
              <div class="workspace-capability-row" aria-label="NDIM scientific capabilities">
                ...
                <div class="workspace-capability"><strong>Scientific modelling</strong>${escapeHtml(modelText)}</div>
                <div class="workspace-capability"><strong>Domains and decisions</strong>${escapeHtml(scopeText)}</div>
              </div>
...
<body>
  ...
  <\/script>
      ...
      function bindCommandCenter() {
        ...
        document.querySelectorAll("[data-workbench-theme-toggle]").forEach((button) => {
        ...
        document.querySelectorAll("[data-workbench-pane]").forEach((button) => {
          button.addEventListener("click", () => {
            state.workspace.activePane = button.dataset.workbenchPane || "";
            render();
          });
        });
        const runButton = $("workbenchRunButton");
        if (runButton) runButton.onclick = () => openStageWorkspace(state.step);
        ...
        document.querySelectorAll("[data-workspace-open-quick]").forEach((button) => {
          button.addEventListener("click", async () => {
            ...
            try {
              await setActiveWorkspace(target, { renderAfter: false });
        ...
        document.querySelectorAll("[data-flow-step]").forEach((button) => {
          button.onclick = () => openStageWorkspace(Number(button.dataset.flowStep));
      ...
      function renderAssistant() {
        ...
        const activeSkills = ndimSkills.filter((skill) => skill.stages.includes(step.id));
        body.innerHTML = `
          ...
          <div class="assistant-card">
            <h3>Active skill${activeSkills.length === 1 ? "" : "s"}</h3>
            ${activeSkills.map((skill) => `
              <p><strong>${escapeHtml(skill.label)}:</strong> ${escapeHtml(skill.summary)}</p>
            `).join("") || `<p>The workspace setup skill is active. Choose or create a project container before modelling begins.</p>`}
          </div>
          <div class="assistant-card">
            <h3>Progress</h3>
        ...
        document.querySelectorAll("[data-workspace-open]").forEach((button) => {
          button.addEventListener("click", async () => {
            if (button.dataset.workspaceOpen !== activeId) {
              await setActiveWorkspace(button.dataset.workspaceOpen, { renderAfter: false });
            }
            startWorkspaceSession(`${workspaceName()} is now the active project workspace.`);
            state.workspace.managerOpen = false;
            state.workspace.startOpen = false;
      ...
      function render() {
        ...
        $("appShell")?.classList.add("df-mode");
        document.body?.classList.add("df-mode-body");
        $("topTitle").textContent = state.workspace.started ? workspaceStageLabel(step) : "NDIM Engine";
        $("topCopy").textContent = state.workspace.started ? workspaceStageSub(step) : "A Scientific Platform for Understanding Narratives and Social Influence.";
