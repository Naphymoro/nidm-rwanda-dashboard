...
for example `https://nidm-engine.pages.dev/engine/?api=http://127.0.0.1:8010`. Without `?api=` it
...
engine to work. A hosted engine (for example Cloud Run) needs no such prompt. The Cloud Run deploy workflow sets this variable
automatically (override it with the `NDIM_ALLOWED_ORIGINS` repository variable).
